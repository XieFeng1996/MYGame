...
@implementation ScrollTextView
{
    ...
    CGFloat _textWidth;
    TextViewScollMove _currentScrollModel;
    TextScrollMoveDirection _currentMoveDirection;
...
- (void)contentMove {
    ...
    if (_textWidth < self.frame.size.width) {//如果字符串长度小于控件宽度，不滚动
        ...
        if (_currentScrollModel == TextScrollWandering) {//往返模式除外
            [self moveWandering];
        }
        return;
    }
